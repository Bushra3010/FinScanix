      ...
      <header className="sticky top-0 z-40 border-b border-border bg-surface/85 backdrop-blur-md">
        <div className="mx-auto flex h-20 w-full max-w-7xl items-center justify-between gap-6 px-5 lg:px-8">
          ...
          <nav className="hidden items-center gap-2 md:flex">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="rounded-lg px-3.5 py-2 text-[14.5px] text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-2.5">
            <ThemeToggle className="h-11 w-11 rounded-xl border-border-strong" />
            <Link
              href="/login"
              className="rounded-lg px-3 py-2 text-[14.5px] text-foreground transition-colors hover:bg-muted"
            >
              Sign in
            </Link>
            <Link
